...
export class RemoteAgentOnboardingService {
  ...
  public renderInstallScript(record: PendingRemoteAgentRecord): string {
    ...
    return [
      ...
      'fi',
      'log_step() {',
      '  echo "[ztools-agent installer] $1"',
      '}',
      '',
      'log_step "detect python runtime"',
      ...
      'log_step "prepare agent directory"',
      'echo "Installing ZTools Linux agent..."',
      'mkdir -p "$AGENT_ROOT"',
      'log_step "resolve remote ip"',
      ...
      'log_step "write runtime env and bootstrap files"',
      ...
      ...(installHookTemplate
        ? [
            'log_step "run install hook template"',
        ...
        : []),
      'log_step "start remote agent runtime"',
      ...
      'fi',
      'sleep 1',
      'log_step "register remote agent back to desktop host"',
      ...
      ')"',
      'if [ "$DEBUG_MODE" -eq 1 ]; then',
      '  curl -v -X POST "$AGENT_REGISTER_URL" \\',
