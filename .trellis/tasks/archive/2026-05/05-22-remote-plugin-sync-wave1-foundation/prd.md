...
- [x] 插件元数据新字段完成共享层定义和归一化
- [x] 本地安装、开发态、内置插件三条链路都能携带新元数据字段
- [x] 远程插件仓共享类型与基础 upsert/empty helper 完成
- [x] 可分发判定函数能够返回布尔结果和不可分发原因
- [x] 为上述纯逻辑补足单元测试
- [x] 现有老插件在不声明新远程字段时仍保留原有本地行为
- [x] 平台已明确“未入远程仓不可直接同步”与“不支持远程分发时入口隐藏/置灰”的契约边界
- [x] 平台已明确“支持远程分发”的最小条件集合，并能稳定据此判定
