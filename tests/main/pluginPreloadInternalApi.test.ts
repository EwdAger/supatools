...
describe('plugin preload internal api bridge', () => {
  ...
  it('exposes deleteRemoteAgent for internal plugin runtimes', async () => {
    require(preloadPath)

    const internalApi = (globalThis as any).window.ztools?.internal

    expect(internalApi?.deleteRemoteAgent).toBeTypeOf('function')

    await internalApi.deleteRemoteAgent('agent-1')

    expect(ipcInvoke).toHaveBeenCalledWith('internal:remote-agent-delete', 'agent-1')
  })
